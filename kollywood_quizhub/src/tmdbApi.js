...
export async function fetchKollywoodMovies(page = 1) {
  ...
  const url = `${TMDB_API_BASE}/discover/movie` +
    `?api_key=${TMDB_API_KEY}&with_original_language=ta&sort_by=popularity.desc&page=${page}`;
...
export async function searchKollywoodMovies(query, page = 1) {
  const url = `${TMDB_API_BASE}/search/movie` +
    `?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(query)}&with_original_language=ta&page=${page}`;
